...
// 播放功能初始化
function audioInit() {
    var xtAudio = $("#xtplayer")[0];
    // 应用初始音量
    xtAudio.volume = volume_bar.percent;
    // 绑定歌曲进度变化事件
    xtAudio.addEventListener('timeupdate', updateProgress);
    xtAudio.addEventListener('play', audioPlay); // 开始播放了
    xtAudio.addEventListener('pause', audioPause);   // 暂停
    xtAudio.addEventListener('ended', nextMusic);   // 播放结束
    xtAudio.addEventListener('error', audioErr);   // 播放器错误处理
}
...
function updateProgress(){
    ...
    // 同步进度条
	music_bar.goto($("#xtplayer")[0].currentTime / $("#xtplayer")[0].duration);
    // 同步歌词显示	
	scrollLyric($("#xtplayer")[0].currentTime);
}
...
function play(music) {
    ...
    $("#xtplayer").attr("src", music.mp3Url);   // 更改播放器音乐url
    $("#xtplayer")[0].play();   // 开始播放
}

// 我的要求并不高，保留这一句版权信息可好？
// 保留了，你不会损失什么；而保留版权，是对作者最大的尊重。
console.info('欢迎使用 xtOnlinePlayer!\n当前版本： v2.0 \n作者：mengkun(http://xtblog.cn)\n歌曲来源于：网易云音乐(http://music.163.com/)\nGithub：https://github.com/mengkunsoft/xtOnlineMusicPlayer');

// 音乐进度条拖动回调函数
function mBcallback(newVal) {
    var newTime = $("#xtplayer")[0].duration * newVal;
    // 应用新的进度
    $("#xtplayer")[0].currentTime = newTime;
...
var initProgress = function(){  
    // 初始化播放进度条
    music_bar = new xtpgb("#music-progress", 0, mBcallback);
    // 初始化音量设定
    var tmp_vol = playerReaddata('volume');
    tmp_vol = tmp_vol? tmp_vol: (isMobile.any()? 1: xtPlayer.volume);
    volume_bar = new xtpgb("#volume-progress", tmp_vol, vBcallback);
};  

// xt进度条插件
// 进度条框 id，初始量，回调函数
xtpgb = function(bar, percent, callback){  
    this.bar = bar;
...
xtpgb.prototype = {
    // 进度条初始化
    init : function(){  
        var xt = this,mdown = false;
        // 加载进度条html元素
        $(xt.bar).html('<div class="xtpgb-bar"></div><div class="xtpgb-cur"></div><div class="xtpgb-dot"></div>');
        // 获取偏移量
        xt.minLength = $(xt.bar).offset().left; 
        xt.maxLength = $(xt.bar).width() + xt.minLength;
        // 窗口大小改变偏移量重置
        ...
        // 监听小点的鼠标按下事件
        $(xt.bar + " .xtpgb-dot").mousedown(function(e){
            e.preventDefault();    // 取消原有事件的默认动作
        });
        // 监听进度条整体的鼠标按下事件
        $(xt.bar).mousedown(function(e){
            if(!xt.locked) mdown = true;
            barMove(e);
        });
        // 监听鼠标移动事件，用于拖动
        ...
        function barMove(e) {
            if(!mdown) return;
            var percent = 0;
            if(e.clientX < xt.minLength){ 
                percent = 0; 
            }else if(e.clientX > xt.maxLength){ 
                percent = 1;
            }else{  
                percent = (e.clientX - xt.minLength) / (xt.maxLength - xt.minLength);
            }
            xt.callback(percent);
            xt.goto(percent);
            return true;
    ...
    goto : function(percent) {
        ...
        this.percent = percent;
        $(this.bar + " .xtpgb-dot").css("left", (percent*100) +"%"); 
        $(this.bar + " .xtpgb-cur").stop().animate({width: (percent*100)+"%"}, 200);
        return true;
    },
    // 锁定进度条
    lock : function(islock) {
        if(islock) {
            this.locked = true;
            $(this.bar).addClass("xtpgb-locked");
        } else {
            this.locked = false;
            $(this.bar).removeClass("xtpgb-locked");
        }
        return true;
    }
};  

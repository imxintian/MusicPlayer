...
$(function(){
    ...
    $("#music-info").click(function(){
        ...
        "<br><span class='info-title'>时长：</span>" + formatTime($("#xtplayer")[0].duration) + 
...
function download(music) {
    var tmpUrl = xtPlayer.api + "?types=download&url=" + music.mp3Url + "&name=" + urlEncode(music.musicName) + "%20-%20" + urlEncode(music.artistsName);
    window.open(tmpUrl);
}
...
function loadList(list) {
    ...
    if(i == 0) {
        ...
        if(rem.playlist === undefined) {
            // 未曾播放过
            if(xtPlayer.autoplay == true) pause();  // 设置了自动播放，则自动播放
        } else {
...
function initList() {
    ...
    for(var i=1; i<musicList.length; i++) {
        ...
        if(i == 1) {    // 正在播放列表
            ...
            if(tmp_item) {  // 读取到了正在播放列表
                ...
                xtPlayer.defaultlist = 1;   // 默认显示正在播放列表
            } else {
    ...
    // 首页显示默认列表
    if(xtPlayer.defaultlist >= musicList.length) xtPlayer.defaultlist = 1;  // 超出范围，显示正在播放列表
    
    if(musicList[xtPlayer.defaultlist].isloading !== true)  loadList(xtPlayer.defaultlist);
